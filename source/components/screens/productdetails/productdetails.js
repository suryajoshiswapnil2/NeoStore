...
export default class ProductDetails extends Component {
    ...
    renderImages = () => {
        ...
        JSX.push(
        <TouchableOpacity onPress={()=>this.setState({curImg: element.image})}>
            <Image source={{uri: element.image}}  style={{width: 105, height: 90, resizeMode: 'contain', borderColor: 'gray', borderWidth: 1, marginRight:10}}/>
        </TouchableOpacity>    
        )})
        return JSX
    }
    ...
    render(){
        ...
        return(
            ...
            <View style={styles.container}>
            <StatusBar barStyle = 'dark-content' hidden={false} />
            <CustomHeader leftIcon='chevron-left' style={{fontSize: 19,}} leftAction={ () => { this.props.navigation.goBack()}} title={this.state.data.name} rightIcon='search'/>
            <View style={styles.mainContainer}>  
              <View>
              <ScrollView style={{marginBottom: 70}}>
                <View style={styles.header}>
                    <Text style={styles.name}>{this.state.data.name}</Text>
                    <Text style={styles.category}>Category - {this.state.data.product_category_id}</Text>
                ...
                <View style={styles.detailContainer}>
                   <View style={styles.imageHolder}>
                       <View style={{flexDirection: 'row', width: '85%', justifyContent: 'space-between', marginTop: 15, marginBottom: 5}}>
                            <Text style={{fontSize:25, color: '#ff0000', fontWeight: '500' }}>Rs. {this.state.data.cost}</Text>
                            <TouchableOpacity onPress={() => this.shareData()}>
                                <Feather name='share-2' size={25} color='gray'/>
                            </TouchableOpacity>
                       </View>
                       <View style={styles.mainImage}>
                           <Image source={{uri: this.state.curImg}}  style={styles.selectedImage}/>
                       </View>
                ...
                <View style={styles.footer}>
                    <TouchableOpacity style={[styles.button, {backgroundColor:'red'}]}  onPress={() => { this.setState( { isBuying: true}) }}>
                        <Text style={styles.buttonText} >BUY NOW</Text>
                    </TouchableOpacity>
                    <TouchableOpacity  style={styles.button} onPress={() => { this.setState( { isRating: true}) }}>
            ...
            <TouchableWithoutFeedback onPress={() =>{Keyboard.dismiss(), this.setState({isBuying: false})}}>
                <View style={{flex:1, backgroundColor:'rgba(0,0,0,0.3)', justifyContent: 'center', alignItems: 'center'}}>
                    <View onTouchEnd={ () => this.setState({isBuying: true})} style={{width:'90%',justifyContent: 'center', alignItems: 'center', height: '70%', margin: 50, backgroundColor: 'white', borderRadius: 10}}>
                    ...
                    <TextInput
                        ...
                        autoCorrect={false}
                        maxLength={1}
                        style={{width: 80, height: 40, borderColor: 'green', borderWidth: 2, padding: 10, margin: 20}}
                        keyboardType='numeric'
                        onChangeText={ (input) => {this.setState({product_quantity: input})} }
                    /> 
                    <TouchableOpacity style={[styles.button, {backgroundColor:'red', width: 180}]}  onPress={() => {this._addToCart()}}>
                        <Text style={[styles.buttonText, {fontWeight: 'bold', textAlign:'center'}]} >SUBMIT</Text>
                    </TouchableOpacity>                
                    </View>
                </View>
            ...
            <TouchableWithoutFeedback onPress={() =>{  this.setState({isRating: false})}}>
                <View style={{flex:1, zIndex: 2147483647, backgroundColor:'rgba(0,0,0,0.3)', justifyContent: 'center', alignItems: 'center'}}>
                    <View  onTouchEnd={ () => this.setState({isRating: true})}  style={{width:'90%',justifyContent: 'center', alignItems: 'center', height: '70%', margin: 50, backgroundColor: 'white', borderRadius: 10}}>
                    <Text style={styles.nameModal}>{this.state.data.name}</Text>
                    <Image source={{uri: this.state.curImg}}  style={styles.selectedImage}/>
                    <Rating
                    type='custom'
                        ...
                        ratingColor='#ffba00'
                        />
                    <TouchableOpacity style={[styles.button, {backgroundColor:'red', width: 230, }]}  onPress={() => { this._postRating() }}>
                        <Text style={[styles.buttonText,{ fontWeight: 'bold'}]} >RATE NOW</Text>
                    </TouchableOpacity>                
                    </View>
                </View>
            </TouchableWithoutFeedback>
